...
export default function TempatReservasiTable() {
  const [dataList, setDataList] = useState<TempatReservasi[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");

  const fetchData = async () => {
    try {
      const json = await fetchWithToken("/api/tempatReservasi");
      setDataList(json);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Terjadi kesalahan");
    } finally {
  ...
  const handleDelete = async (id: number) => {
    const konfirmasi = confirm("Yakin ingin menghapus tempat ini?");
    if (!konfirmasi) return;

    try {
      const json = await fetchWithToken(`/api/tempatReservasi/${id}`, {
        method: "DELETE",
      });

      alert(json.message || "Berhasil dihapus");
      setDataList((prev) => prev.filter((item) => item.tempat_reservasi_id !== id));
    } catch (err) {
  ...
  return (
    <>
      <Breadcrumb pageName="Tempat Reservasi" mapName="Page Tempat Reservasi" />
      <div className="rounded-[10px] bg-white shadow-1 dark:bg-gray-dark dark:shadow-card p-6">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-dark dark:text-white">Daftar Tempat</h1>
          <Link
            href={withAdminPrefix(`/tempat_reservasi/create`)}
            className="rounded-md bg-primary px-4 py-2 text-white hover:bg-primary/90"
          >
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-t text-base [&>th]:h-auto [&>th]:py-3 sm:[&>th]:py-4.5">
              <TableHead className="min-w-[40px] pl-5">ID</TableHead>
          ...
          <TableBody>
            {dataList.map((item) => (
              <TableRow key={item.tempat_reservasi_id} className="text-base font-medium text-dark dark:text-white">
                <TableCell className="pl-5">{item.tempat_reservasi_id}</TableCell>
                ...
                <TableCell className="text-right pr-5">
                  <div className="flex justify-end gap-x-3.5">
                    <Link
                      href={withAdminPrefix(`/tempat_reservasi/show/${item.tempat_reservasi_id}`)}
                      className="hover:text-primary"
                      aria-label="Lihat"
                    >
                      <PreviewIcon />
                    ...
                    <Link
                      href={withAdminPrefix(`/tempat_reservasi/edit/${item.tempat_reservasi_id}`)}
